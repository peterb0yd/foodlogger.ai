...
export const parseFoodItemLogData = async (transcription: string) => {
	try {
			...
			messages: [
				{
					role: 'system',
					content: `
                    Your only job is to decipher the sentence from the user and create a food item log from it. You need to extract the food item name, preparation method, quantity and unit from the sentence. The preparation method must be one of these case sensitive values: ${preparationMethods.join(', ')}. If you don't understand the preparation method or can't find a match to one of those values, omit the field. The unit must be one of these case sensitive values: ${unitValues.join(',')}. If they say "spoon" match it to "TABLESPOON". If you don't understand the unit or can't find a match, set unit to "NONE". Quantity is a float alue. You can use 0.5 if they say "half" or 2.5 if they say "two and a half". Respond in JSON format as shown in the examples.
		...
		return JSON.parse(response.choices[0].message.content as string) as IFoodLogItemTranscriptionOutput;
	} catch (error) {
